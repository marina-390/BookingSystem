...
router.post("/", resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
   ...
   // ⭐ REQUIRED LOG ENTRY FOR VALIDATION FAILURES ⭐ 
   await logEvent({ 
    actorUserId: null, 
    message: `RESOURCE_CREATE_VALIDATION_FAILED: ${JSON.stringify( 
  ...
  try {
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE_CREATE_BLOCKED_DUPLICATE: name="${resourceName}" (HTTP 409)`,
        entityType: "resource",
        entityId: null,
      });
