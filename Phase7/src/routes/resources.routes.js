...
router.post("/", requireAuth, resourceValidators, async (req, res) => {
  // Only managers can create resources
  if (req.user.role !== 'manager' && req.user.role !== 'administrator') {
    return res.status(403).json({ ok: false, error: "Only managers can create resources" });
  }
...
/* =====================================================
   ...
   - Manager only
===================================================== */
router.put("/:id", requireAuth, resourceValidators, async (req, res) => {
  // Only managers can update resources
  if (req.user.role !== 'manager' && req.user.role !== 'administrator') {
    return res.status(403).json({ ok: false, error: "Only managers can update resources" });
  }
...
router.delete("/:id", requireAuth, async (req, res) => {
  ...
  if (req.user.role !== 'manager' && req.user.role !== 'administrator') {
    return res.status(403).json({ ok: false, error: "Only managers can delete resources" });
  }
